  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {severities.map(severity => (
          <div key={severity} className={`${getSeverityColor(severity)} border-l-4 rounded-lg p-4`}>
            ...
            <div className="text-3xl font-bold text-gray-900">{severityCounts[severity]}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-6">
        ...
        {/* Severity Filter */}
        <div className="mb-4">
          <div className="text-sm font-medium text-gray-700 mb-2">Severity</div>
            ...
            {filteredIssues.map((issue: any, idx: number) => (
              <div key={idx} className={`border-l-4 p-4 rounded ${getSeverityColor(issue.severity)}`}>
                <div className="flex items-start justify-between mb-2">
                  <div className="font-semibold text-gray-900 flex-1">{issue.message}</div>
                ...
                {issue.suggested_fix && (
                  <div className="text-sm text-green-700 mt-2 bg-white p-3 rounded">
